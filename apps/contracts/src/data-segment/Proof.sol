...
contract Proof {
  ...
  function computeExpectedAuxData(
    ...
    require(
      assumedSizePa == assumedSizePa2,
      "aggregator's data size doesn't match"
    );

    validateIndexEntry(ip, assumedSizePa2);
    return
      InclusionAuxData(assumedCommPa.pieceCommitmentToCid(), assumedSizePa);
  }
  ...
  function computeExpectedAuxDataWithDeal(
    ...
    InclusionAuxData memory inclusionAuxData = computeExpectedAuxData(
      ip,
      verifierData
    );
  ...
  function validateInclusionAuxData(
    ...
    require(
      dealDataCommitment.size == inclusionAuxData.sizePa,
      "Deal size doesn't match"
    );
  }
  ...
  function indexAreaStart(uint64 sizePa) internal pure returns (uint64) {
    return
      uint64(sizePa) -
      uint64(maxIndexEntriesInDeal(sizePa)) *
      uint64(ENTRY_SIZE);
  }

  // maxIndexEntriesInDeal returns the maximum number of index entries that can be stored in a deal of the given size.
  function maxIndexEntriesInDeal(
    uint256 dealSize
  ) internal pure returns (uint256) {
    uint256 res = (uint256(1) <<
      uint256(log2Ceil(uint64(dealSize / 2048 / ENTRY_SIZE)))); //& ((1 << 256) - 1);
    if (res < 4) {
      return 4;
    }
    return res;
